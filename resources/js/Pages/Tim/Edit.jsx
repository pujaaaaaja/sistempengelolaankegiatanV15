// FUNGSI: Halaman untuk menampilkan formulir edit tim.
// ===================================================================================
...
export default function Edit({ auth, tim, pegawais }) {
  const { data, setData, post, errors, processing } = useForm({
    nama_tim: tim.data.nama_tim || '',
    // Ambil ID anggota tim saat ini
    user_ids: tim.data.users.map(user => user.id) || [],
    _method: 'PUT',
  });
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    post(route('tim.update', tim.data.id));
  };

  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Tim "{tim.data.nama_tim}"</h2>}
    >
      <Head title="Edit Tim" />
      <div className="py-12">
                  ...
                  {pegawais.data.map((pegawai) => (
                    <div key={pegawai.id} className="flex items-center">
                      <Checkbox
                        ...
                        // Tandai checkbox jika ID pegawai ada di dalam data.user_ids
